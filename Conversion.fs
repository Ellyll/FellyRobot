...
let regSingularUnit = Regex(@"^\s*(\d+(\.\d+)?)\s*(f|F|c|C|k|K|g|kg|st|lb|oz)\s*$")
let (|SingularUnit|_|) input : Measurement option =
    if regSingularUnit.IsMatch(input) then
        let valueString = regSingularUnit.Replace(input, "$1")
        let unitString = regSingularUnit.Replace(input, "$3")
